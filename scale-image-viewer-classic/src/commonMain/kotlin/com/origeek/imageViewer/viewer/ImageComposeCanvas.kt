...
import com.jvziyaoyao.scale.image.sampling.CommonBigDecimal
import com.jvziyaoyao.scale.image.sampling.CommonRoundingMode
...
import com.jvziyaoyao.scale.image.sampling.recycle
...
fun ImageComposeCanvas(
    ...
    val needRenderHeightTexture by remember(key1 = bSize) {
        derivedStateOf {
            ...
            CommonBigDecimal(samplingDecoder.decoderWidth)
                .multiply(CommonBigDecimal(samplingDecoder.decoderHeight)) > CommonBigDecimal(bSize.height)
                .multiply(CommonBigDecimal(bSize.width))
        }
    }
    ...
    LaunchedEffect(key1 = Unit) {
        samplingDecoder.startRenderQueue {
            // 解码器解码一个，就更新一次时间戳
            renderUpdateTimeStamp = Clock.System.now().toEpochMilliseconds()
        }
    }
    ...
    fun updateRenderList() {
        ...
        scope.launch(Dispatchers.IO) {
            synchronized(samplingDecoder.renderQueue as SynchronizedObject) {
                insertList.forEach {
    ...
    LaunchedEffect(key1 = rSize, key2 = rectW, key3 = rectH) {
        // 可视区域面积
        val rectArea =
            CommonBigDecimal(rectW.toDouble()).multiply(CommonBigDecimal(rectH.toDouble()))
        // 实际大小面积
        val realArea = CommonBigDecimal(rSize.width).multiply(CommonBigDecimal(rSize.height))
        // 被除数不能为0
        if (realArea.toFloat() == 0F) return@LaunchedEffect
        // 计算实际面积的可视率
        val renderAreaPercentage =
            rectArea.divide(realArea, 2, CommonRoundingMode.HALF_EVEN).toFloat()
